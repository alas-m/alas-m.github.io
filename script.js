...
const API_TOKEN = 'ut_o7HFZrBTAcsY8ZctHo8cwUsduv3AhZ7q9owsLZL7';
const COUNTER_ID = 'first-counter-1708';

async function initGlobalVisitCounter() {
    const counterElement = document.getElementById('visit-counter');
    const api_url = `https://api.counterapi.dev/v1/counter/${COUNTER_ID}/increment`;
    ...
    try {
        ...
        if (counterElement) {
            counterElement.textContent = `System Access Log: ${data.count}`;
        }
        
    } catch (error) {
        console.error("Error fetching global visit count:", error);
        if (counterElement) {
            counterElement.textContent = `System Access Log: Error`;
        }
    }
}
